...
import it.univpm.turista.utilities.operation;
...
public class Statistics {
	...
	public static double variazione(double[] val) {
		return operation.max(val) - operation.min(val);
	...
	public static double avg(double[] val) {
		return operation.sum(val) / val.length;
	}
	...
	public static double per_avg(double[] per) {
		return operation.sum(per) / per.length;
	}
